...
**Scope-beslut (kommunicerat, sedan utvidgat — se uppföljning nedan):** modell-lagrets djupa felmeddelanden hölls först på svenska; de översattes i en direkt uppföljning (samma dag).

**Uppföljning (segment 28b) — modell-fel översatta:** På användarens begäran routades de realistiskt synliga modell-felen genom samma `t()`: **FlipError** (bayesReversal, 10 meddelanden), **DocumentError** (document.ts topp-nivå: ogiltig JSON, okänt format, display_mode/utility/split/right_edited, trasig variabelgrupp), **VariableConflictError** + variable-rename-fel, **BackwardFillError** (5), och **UtilityDomainError** (3). Modell-filer importerar nu `t` från `../i18n` (global språk-state — ren funktions-logik oförändrad, bara meddelandetexten lokaliseras). `typeSv`/`typeLabel` ersatta av `t().nodeTypeWord`. **Kvar på svenska (medvetet):** document.ts:s ~15 *djupa fält-valideringsmeddelanden* (triggas bara av handkorrupt JSON — ser en normal användare aldrig) och de tre **redan engelska** interna fel-klasserna (`ProbabilitySumError`/`CyclicTreeError`/`AmbiguousConditionalProbabilityError`, som fångas och visas som översatta korta varningar, aldrig råa). backwardFill/utility-exponential-felen var redan engelska → fick svenska översättningar (sv) så svenska-läget nu är fullständigt; 3 test-assertions som låste engelska strängar uppdaterade till svenska. **Bugfix:** `state.flipError` är en cachad sträng — vid språkbyte om-härleds den nu via `regenerateRight()` (bara när ett flip-fel är aktivt, så inget giltigt högerträd rörs). Meddelanderadens transienta text lämnas (historiskt event; nästa åtgärd sätter en färsk på nya språket). 258 gröna (+1 modell-fel-språktest), tsc + build rena, live-verifierat (flip-fel SV↔EN↔SV utan kvarhängande språk).
